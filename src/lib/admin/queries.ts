...
export type EstadoSuscripcion = 'trial_activo' | 'trial_vencido' | 'basico' | 'pro';
...
export type EmpresaResumen = {
  ...
  diasRestantes: number | null; // solo trial
...
function mapRow(r: RpcRow): EmpresaResumen {
  ...
  if (plan === 'pro') {
    ...
    estado = activo ? 'trial_activo' : 'trial_vencido';
    diasRestantes = venceMs ? Math.max(0, Math.ceil((venceMs - ahora) / DAY)) : 0;
  }
...
export function calcOverview(empresas: EmpresaResumen[]): OverviewStats {
  ...
  for (const e of empresas) {
    ...
    if (e.plan === 'basico') mrr += PRECIOS.basico;
    if (e.plan === 'pro') mrr += PRECIOS.pro;
    if (e.activa) activas++;
    if (e.ventasCount === 0) zombies++;
  }

  const pagando = porPlan.basico + porPlan.pro;
